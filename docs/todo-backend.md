...
- [x] 보관 기간 경과 이벤트 삭제 잡 — `ops/retention.ts`, 6시간 인터벌 + `pnpm retention` 스크립트(cron용). events만 삭제(이슈 요약 영구)
- [x] 오래된 릴리즈의 소스맵 아티팩트 정리 잡 — `deleteOldArtifacts` (cascade로 아티팩트 삭제)
- [x] 헬스체크 엔드포인트 (`/healthz` — DB 연결 포함) — STEP 2에서 선구현
- [x] 자기 자신의 에러 로깅 — `setErrorHandler`(5xx는 로그, 클라이언트엔 일반 메시지로 스택 유출 방지). 자기 자신을 SDK로 추적하지 않아 무한 루프 없음
